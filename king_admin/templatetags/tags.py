...
def build_table_row (request,obj,admin_class):
    ...
    for index,column in enumerate(admin_class.list_display):
        fieid_obj = obj._meta.get_field(column)
        if fieid_obj.choices:
...
def render_page_ele (loop_counter, query_sets,filter_conditions):
    ...
    for k,v in filter_conditions.items():
        filters += "&%s=%s" %(k,v)
    if loop_counter <3 or loop_counter>query_sets.paginator.num_pages-2 : # 代表这是前两项
        ele_class = ""
...
def build_paginators (query_sets ,filter_conditions,previous_orderby,search_text):
    ...
    for page_num in query_sets.paginator.page_range:
        ...
        if page_num < 3 or page_num > query_sets.paginator.num_pages-2 or  \
                abs(query_sets.number - page_num) <= 2 :   #代表前两页或者最后两页
        ...
        # elif abs(query_sets.number - page_num) <= 1: #判断最后一页
        #     ele_class = ""
        #     if query_sets.number == page_num:
        #         added_dot_ele = False
        #         ele_class = "active"
